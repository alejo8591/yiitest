...
class ProjectUserForm extends CFormModel
{
	...
	{
		return array(
			// username and password are requires
	 ...
	 * If valid, it will also make the association 
	 * between the user, role and project
	...
	public function verify($attribute, $params)
	{
		if(!$this->hasErrors())
		{
			...
			{
				$this->addError('username', 'This user has alredy been added to the Project.')
			}
			else
			{
				...
				$bizRule= 'return isset($params["project"] && $param["project"]->isUserInRole("'.$this->role.'"))';
